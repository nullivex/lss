...
class Login {
	...
	public static function requireManager(){
		...
		if(self::$user['is_manager'] == 0) throw new Exception('permission denied <a href="'.Url::home().'">return</a>');
	}
	...
	public function adminNav($user){
		Tpl::_get()->setConstant('admin_nav',Tpl::_get()->parse('global','staff_nav',array(),true));;
		if(!isset($user['is_manager']) || !$user['is_manager']) return;
		Tpl::_get()->setConstant('admin_nav',Tpl::_get()->parse('global','admin_nav',array(),true));
	}
