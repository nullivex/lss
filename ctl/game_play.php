...
//update metrics
$games->addMetrics($_game['game_id']);
...
//related games
$i = 0;
foreach($games->relatedGames($_game['game_id'],Config::get('list','play_related')) as $game){
	$game['url'] = Url::game_info($game['game_id']);
	$game['row'] = $i % Config::get('list','play_related_row') == 0 ? '</tr><tr>' : ''; $i++;
	$params['related_games'] .= Tpl::_get()->parse('game_info','related_row',$game,true);
}

//featured games
$i = 0;
foreach($games->gamesFeatured(Config::get('list','play_featured')) as $game){
	$game['url'] = Url::game_info($game['game_id']);
	$game['row'] = $i % Config::get('list','play_featured_row') == 0 ? '</tr><tr>' : ''; $i++;
	$params['featured_games'] .= Tpl::_get()->parse('game_info','featured_row',$game,true);
}

//tpl
$tpl = $_game['width'] > Config::get('game','fullpage_min_width') ? 'game_play_full' : 'game_play';

Tpl::_get()->parse('game_play',$tpl,$params);
