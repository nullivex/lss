...
function addValue($def,$name=null,$value=null){
	...
	if(mda_exists($def->data,$value,$name)) throw new Exception('Trying to add a value that already exists');
...
function delValue($def,$name=null,$value=null){
	...
	if(is_null($value)) return mda_del($def->data,$name);
		...
		if(!mda_exists($def->data,$value,$name)) throw new Exception('Trying to delete a value that does not exist');
